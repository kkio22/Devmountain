...
public final class AiConstants {
	...
	public static final String POST_RECOMMENDATION_CONVERSATION_PROMPT = """
		...
		예시 :
		input: 사용자 메세지 : "추천 강의가 마음에 들지 않아요! 더 궁금한 점이 있어요"
				회원 등급 : PRO
		output: 회원님, 추가로 궁금한 점이 있으시면 언제든지 질문해 주세요!
		
		input: 사용자 메세지 : "무료 강의만 다시 보여줄 수 있나요?"
				회원 등급 : GUEST
		output: 비회원이라서 일부 강의만 추천드릴 수 있습니다. 회원 가입을 진행하시면 더 다양한 강의를 안내해 드리겠습니다.
		
		input: 사용자 메세지 : "난이도를 변경해서 다시 추천해주세요"
				회원 등급 : FREE
		output: 난이도 조건을 변경해서 다시 추천해드릴게요! 원하시는 난이도를 말씀해주세요!
		
		[실제 입력]
		  input: 사용자 메시지: {userMessage}
	...
	public static final String RERECOMMENDATION_DETECT_PROMPT = """
		...
		[응답 형식]
		- 반드시 아래 형식에 따를 것
		- output 은 YES 또는 NO 중 하나
		
		[예시]
		input: 사용자 메세지 : 다른 강의도 추천해줘
		output: YES
		
		input: 사용자 메세지 : 이 강의 설명 좀 더 해줘
		output: NO
		
		input: 사용자 메세지 : 초급 강의로 다시 추천해줘
		output: YES
		
		input: 사용자 메세지 : 이 강의는 무료인가요?
		output: NO
		
		input: 사용자 메세지 : 다른 거 볼 수 있을까요?
		output: YES
		
		input: 사용자 메세지 : 추천된 강의 등록은 어떻게 하나요?
		output: NO
		
		input: 사용자 메세지 : 좀 더 저렴한 강의는 없나요?
		output: YES
		
		input: 사용자 메세지 : 입문자용으로 다시 보여주세요.
		output: YES
		""";
}
